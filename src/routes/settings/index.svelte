<script context="module">
  ...
  export async function preload(page, session) {
    ...
    const { isVerified } = user;
		...
		if (!user) {
      session.messages = addMsg('info', 'Please log in to view your settings', session.messages);
...
<script>
  ...
  async function resend(event) {
    ...
    if (response.errors) errorMsgs.set([...$errorMsgs, JSON.stringify(response.errors)]);
		if (!response.errors) {
  ...
  <div class="columns">
    <div class="column is-two-thirds">
      {#if $session.user && !isVerified}
        ...
        <ResendEmailConfirmForm on:resend={resend} email={$session.user.email}/>
      {:else}
        <h1 class="is-size-3">Your Settings</h1>

        {#if !user.facebook}
          <div class="block">
            <FacebookAuth text='Link your Facebook account' appId=process.env.FB_APP_ID
